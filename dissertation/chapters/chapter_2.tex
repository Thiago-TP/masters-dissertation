...
Nem todo editor \LaTeX\ compila automaticamente a lista de abreviaturas e siglas e a lista de símbolos. Para garantir que elas apareçam no documento após a compilação, confira a \cref{sec:compilar}.
...
Para indicar nas referências bibliográgicas as páginas do documento em que cada uma é citada, utilize a opção \texttt{refback} da classe UnB\TeX. Caso não queira indicar as páginas em que cada referência é citada, basta não utilizar a referida opção. %Note que o uso da opção \texttt{refback} pode aumentar o tempo necessário para compilar completamente o documento.
...
Para que sejam geradas as listas de referências, listas de símbolos, remissões internas, dentre outros componentes que um trabalho acadêmico deve ter, é necessário executar, na sequência, \texttt{pdfLaTeX}, \texttt{BibTeX}, \texttt{MakeIndex} e \texttt{pdfLaTeX} mais duas vezes. O \texttt{MakeIndex} deve ser previamente configurado conforme o editor \LaTeX\ utilizado\footnote{Por exemplo, para TeXstudio, veja \url{https://tex.stackexchange.com/questions/27824/using-package-nomencl}}.

No Overleaf, essa tarefa é automatizada. Basta compilar com o \texttt{pdfLaTeX} e toda sequência é realizada. No entanto, devido à restrições de tempo de compilação impostas pelo Overleaf (na versão gratuita), foi adicionado ao UnB\TeX\ o arquivo \texttt{latexmkrc} que, para o Overleaf, limita a dois o número de vezes que o \texttt{pdfLaTeX} é chamado. Assim, para que todas as referências e listas apareçam corretamente no documento, é necessário compilar o documento duas vezes quando o projeto for aberto pela primeira vez. Se o tempo de compilação continuar excessivo ou se a versão paga do Overleaf estiver disponível, é possível editar o arquivo \texttt{latexmkrc} para reduzir ou aumentar o número de vezes que o \texttt{pdfLaTeX} é chamado. Tal alteração impactará no número de compilações necessárias para que o documento fique totalmente pronto. Alternativamente, para reduzir o tempo da primeira compilação, é possível utilizar a opção \texttt{draft} da classe UnB\TeX, que não carrega e nem processa imagens. Nas compilações seguintes, remova a opção \texttt{draft}. Assim, é possível obter o documento completo (incluindo imagens), com tempo de compilação menor, uma vez que parte do processamento já foi realizado.

O arquivo \texttt{latexmkrc} também automatiza a compilação em editores \LaTeX\ instalados em ambientes locais. Neste caso, o arquivo \texttt{latexmkrc} está configurado para executar automaticamente o \texttt{pdfLaTeX} até quatro vezes, o que é suficiente para compilar completamente um documento. No TeXstudio, por exemplo, acesse o menu \texttt{Ferramentas} e escolha as opções \texttt{Comandos} e \texttt{Latexmk}.
